...
public class BookCrudController {
    ...
    //Função responsável pelo endpoint de mostrar livros com paginação
    @GetMapping
    @Operation(summary = "Listar livros usando filtros com paginação")
    public ResponseEntity<Page<BookResponse>> getFilteredBooks(
            @RequestParam(value = "page", defaultValue = "0") Integer page,
            @RequestParam(value = "size", defaultValue = "8") Integer size,
            @RequestParam(value = "orderBy", defaultValue = "title") String orderBy,
            @RequestParam(value = "direction", defaultValue = "ASC") String direction) {
        return ResponseEntity.ok().body(service.getFilteredBooks(page, size, orderBy, direction));
    }
