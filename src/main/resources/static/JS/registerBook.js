...
// Codigo acima para garantir que o campo de quantidade de exemplares seja um valor positivo
...
document.getElementById('RegisterLivro_Form').addEventListener('submit', async function(e) {
    ...
    try {
        // Envia os dados do livro para a API usando o metodo POST.
        const response = await fetch('/books/register', { // add a url da api
            method: 'POST',
            headers: {
        ...
        limpaFormular();
        window.location.href = 'registerBook.html'; // Redireciona para a página registerBook.html.
    } catch (error) {
